...
const ExperienceCard = ({position, company, date}) =>{
    return(
        <Card style={{ width: '23rem', padding: '1.25rem 1rem 0.75rem 1rem',borderRadius: '10px', background: '#072340', color: '#DAE4FB'}} id={ProjectCard.style}>
          <Card.Body style={{ background: '#072340' }}>
            ...
            <Card.Text style={{ background: '#072340',color: '#96A2BC', marginLeft: '2.5rem', fontSize: '1.2rem' }}>
              {company}
            <div style={{color: '#7C7DAC',background: '#072340', fontSize: '1rem'}}><FaCalendarAlt/> {date}</div>
            </Card.Text>
          </Card.Body>
        </Card>
          )
